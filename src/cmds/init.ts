	...
	async handler({ path }) {
		...
		if (await Bun.file(tkTomlPath).exists()) {
			fatalError({
				...
				why: ["I noticed there was already a `tk.toml` file in that folder"],
				hint: [
		...
		printer.write(`- Start the development server with ${chalk.cyan("`tk dev`")}`);
		printer.write(`- Open your project in Roblox, you may have to run ${chalk.cyan("`tk studio`")}`);
		printer.write(`- Dont use AI, you'll never learn, plus, your brain is awesome, why waste it?`);
		printer.dedent();
		printer.gap();
